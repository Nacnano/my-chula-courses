...
if not df_filtered.empty:
    ...
    # Remove outliers: Keep only 0-80th percentile for duration_hours
    lower_bound, upper_bound = df_filtered['duration_hours'].quantile([0.0, 0.8])
    df_filtered = df_filtered[(df_filtered['duration_hours'] >= lower_bound) & (df_filtered['duration_hours'] <= upper_bound)]
    print(f"Shape after calculating positive 'duration_hours' and removing outliers (0-80th percentile): {df_filtered.shape}")
else:
...
print(f"Number of available samples after initial filtering, duration calculation, and outlier removal: {num_available_filtered_samples}")
